...
const RezAI = () => {
  ...
  useEffect(() => {
    ...
    window.rezAPI.onStatus((newStatus: string) => {
      setStatus(newStatus)
    ...
    // 2. Listen for Audio Levels (Visualizer)
    window.rezAPI.onVisualize((level: number) => {
      // Smooth interpolation could go here, but direct setting works for high FPS
      setAudioLevel(level)
    })
  }, [])
  ...
  const handleToggle = async () => {
    ...
    setIsLive(targetState) // Optimistic update
    await window.rezAPI.toggleSession(targetState)
  }

  const handleMute = async () => {
    const targetMute = !isMuted
    setIsMuted(targetMute)
    await window.rezAPI.toggleMute(targetMute)
  }

  // Calculate dynamic scale based on audio level
  // Base scale 1 + (level * 0.5) -> bumps up to 1.5x size
  const orbScale = isLive ? 1 + audioLevel * 0.4 : 1
  ...
  return (
    <div className="relative h-screen w-full bg-black overflow-hidden selection:bg-cyan-500/30">
      ...
      <div className={`absolute top-[-10%] left-[-10%] w-125 h-125 bg-cyan-500/20 rounded-full blur-[120px] transition-all duration-1000 ${isLive ? 'opacity-100 scale-110' : 'opacity-40 scale-100'}`}></div>
      ...
      <div className="relative z-10 h-[calc(100vh-100px)] w-full flex items-center justify-center flex-col gap-8">
        ...
        <div className="relative group cursor-pointer">
          {/* Outer Atmosphere Glow - Dynamic Opacity based on volume */}
          <div
            className={`absolute inset-0 rounded-full blur-[60px] transition-all duration-100
            ${isLive ? 'bg-cyan-500/40' : 'bg-gray-800/20'}`}
            style={{ 
              opacity: isLive ? 0.4 + audioLevel : 0,
          ...
          <div
            className={`w-64 h-64 rounded-full relative flex items-center justify-center transition-all duration-75 ease-out
            bg-gradient-to-b from-gray-800 via-gray-950 to-black
            shadow-[inset_0px_-20px_60px_rgba(0,0,0,0.9),inset_0px_10px_20px_rgba(255,255,255,0.1),0px_0px_50px_rgba(0,0,0,0.5)]
            ${isLive ? 'shadow-cyan-500/20 border border-cyan-500/30' : 'border border-white/5'}
            `}
            style={{ transform: `scale(${orbScale})` }} 
          >
            {/* The Inner "Core" Gradient - Reacts heavily to volume */}
            <div
              className={`absolute inset-4 rounded-full opacity-60 bg-gradient-to-tr 
              ${isLive ? 'from-cyan-600/20 via-transparent to-cyan-600/20' : 'from-gray-700/10 via-transparent to-gray-800/10'}`}
              style={{ opacity: isLive ? 0.2 + audioLevel : 0.1 }}
            ></div>

            <div className="absolute top-8 left-12 w-20 h-10 bg-white/5 rounded-full blur-md transform -rotate-45"></div>

            <div className="flex flex-col items-center justify-center gap-2">
              <span className={`text-4xl font-black tracking-widest transition-colors duration-500 
                ${isLive ? 'text-cyan-100 drop-shadow-[0_0_15px_rgba(6,182,212,0.8)]' : 'text-gray-600'}`}>
                REZ AI
              </span>
              <span className={`text-xs font-mono tracking-[0.3em] uppercase transition-colors duration-500
        ...
        <div className="text-center space-y-2">
          <p className={`text-sm font-mono tracking-wider transition-all duration-500
             ${isLive ? 'text-cyan-400' : 'text-gray-500'}`}>
             {isLive ? (audioLevel > 0.05 ? '/// SPEAKING ///' : '/// LISTENING ///') : '/// OFFLINE ///'}
          </p>
        </div>
      </div>
      ...
      <div className="fixed bottom-8 left-1/2 transform -translate-x-1/2 z-50">
        <div className="flex items-center gap-4 bg-gray-900/80 backdrop-blur-2xl border border-white/10 rounded-2xl px-3 py-3 shadow-[0_0_40px_rgba(0,0,0,0.5)] ring-1 ring-white/5">
          ...
          <button
            ...
            ${!isLive ? 'opacity-30 cursor-not-allowed text-gray-500' : 
              isMuted 
      ...
      <div className="fixed top-8 right-8 z-50">
        <div className={`flex items-center gap-3 px-4 py-2 rounded-full border backdrop-blur-md transition-all duration-500
          ...
          <span className={`text-xs font-mono font-bold tracking-widest ${isLive ? 'text-cyan-100' : 'text-gray-500'}`}>
            {status}
          </span>
          <div className={`w-2 h-2 rounded-full ${isLive ? 'bg-cyan-400 animate-pulse' : 'bg-gray-600'}`}></div>
        </div>
      </div>
    </div>
  )
